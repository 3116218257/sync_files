...
class NeuralEFCLR(nn.Module):
    def __init__(self, args):
        ...
        #pretrained = torchvision.models.resnet50(zero_init_residual=True)
    ...
    def forward(self, y1, y2, t_input, index, labels=None):
        ...
        K_ = labels.view(-1, 1) == labels.view(1, -1)
        new_kernel = K_ / K_.sum(-1) ** 2
        ...
        if self.args.positive_def:
            ...
            if self.steps % 20 == 1:
                ...
                datass = np.array(psi1.detach().cpu())
                labs = np.array(labels.detach().cpu())

                k = 10
                kmeans = KMeans(n_clusters=k, random_state=42)
                cluster_labels = kmeans.fit_predict(datass)

                label_mapping = {}
                for cluster_label in set(cluster_labels):
                    mask = (cluster_labels == cluster_label)
                    assigned_labels = labs[mask]
                    most_common_label = np.argmax(np.bincount(assigned_labels))
                    label_mapping[cluster_label] = most_common_label

                predicted_labels = [label_mapping[label] for label in cluster_labels]

                accuracy = accuracy_score(labs, predicted_labels)
                print("聚类结果的正确率：", accuracy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--proj_dim', default=[2048, 256], type=int, nargs='+')
    ...
    cifar_dataset = CIFAR10Dataset(root_dir='./data', train=True, transform=transform)
    cifar_dataset_test = CIFAR10Dataset(root_dir='./data', train=False,
                                        transform=transform)
    ...
    Schedule = diffusion_process.GaussianDiffusion(timestep=1000)
    for epoch in range(args.num_epochs):
        model.train()
        for (image1, image2), labels, classes_name in tqdm(train_data_loader,
            ...
            ypn2 = Schedule.q_sample(image1, rand_index)
            ...
            # print(rand_index[0])
            # noised = Schedule.q_sample(image1, rand_index)
            # if(index[0]<10000):
            #     tmp1 = image1
            #     print(tmp1.shape)
            #     tmp = tmp1[0]
            ...
            #     tmp = tmp.byte()
            #     image = transforms.ToPILImage()(tmp)
            #     image.save(f"./noised.jpg")
            # assert index[0] == 10000
        ...
        #         # rand_index = torch.randint(0, 10, size=(1,))##########use clean data temporary with no embedding

        #         # index = torch.full((image1.shape[0],), int(rand_index))
        #         rand_index = torch.randint(0, 1000, size=(image1.shape[0],))
        #         # index = torch.full((image1.shape[0],), int(rand_index))
        #         index = rand_index
