...
class DiffusionEF(nn.Module):
    def __init__(self, args, image_size, num_timesteps, num_classes, learned_sinusoidal_dim=16):
        ...
        for m in self.encode_unet.modules():
        ...
        nn.init.xavier_uniform_(self.online_clf_head.weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--time_dependent', default=True, action='store_true')
    ...
    ckpt_path = './ckpt/'
    # model = torch.load(ckpt_path + "model20.pt")
    ...
    for epoch in range(args.num_epochs):
        ...
        for (image1, image2), labels, classes_name in tqdm.tqdm(train_data_loader, desc=f"Epoch {epoch+1}/{args.num_epochs}", unit="batch"):
            n_steps += 1
            ...
            #     for params in model.online_clf_head.parameters():
            #         params.requires_grad = False
            #Loss = loss + reg * args.alpha + cls_loss
            ...
            if n_steps%20 ==0:
                print('Epoch: {}, t: {}, LR: {:.4f}, Loss: {:.4f}, Reg: {:.4f}, CLS_Loss: {:.4f}, Train acc: {:.2f}%'.format(
                    epoch+1, t.item(), lr, losses / n_steps, regs / n_steps, cls_losses / n_steps_clf, 100 * accs / n_steps_clf), file=logfile)
                print('Epoch: {}, t: {}, LR: {:.4f}, Loss: {:.4f}, Reg: {:.4f}, CLS_Loss: {:.4f}, Train acc: {:.2f}%'.format(
                    epoch+1, t.item(), lr, losses / n_steps, regs / n_steps, cls_losses / n_steps_clf, 100 * accs / n_steps_clf))
                loss_his.append(losses / n_steps)
                cls_loss_his.append(cls_losses / n_steps_clf)

        if epoch % 5 == 4:
            torch.save(model, ckpt_path + f'model{epoch+1}.pt')
        ...
        if epoch % 2 == 0:
            ...
            test_image1 = test_image1.permute(0, 3, 1, 2)
            test_image2 = test_image2.permute(0, 3, 1, 2)
